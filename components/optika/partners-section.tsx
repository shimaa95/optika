...
export function PartnersSection({
  ...
  const taglineText = tagline?.trim() || 'Exceptional Optical Solutions'
  const headlineText = headline?.trim() || 'Partners integrated solutions'
  const bodyText =
    body?.trim() ||
  ...
  const linkLabel = ctaLabel?.trim() || 'Become a Partner'
  const imageSrc = image != null ? urlFor(image).width(1600).url() : '/whatwedo.jpeg'
  const imageAlt = headlineText
    ...
    <section className="relative px-6 lg:px-20 xl:px-24 py-16 lg:py-24 bg-black">
      <div className="flex flex-col-reverse lg:flex-row items-center w-full gap-12 lg:gap-16 xl:gap-24">
        ...
        <div className="flex flex-col w-full lg:w-[35%] xl:w-[32%] justify-center pr-0 lg:pr-8">
          ...
          <Link href={linkHref} className='cursor-pointer w-fit group'>
